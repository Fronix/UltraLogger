...
public class MainLogger extends JavaPlugin{
	...
	public Translater t;
	private String pref = "[UltraLogger] v1.5.2 ";
	...
	public void onEnable() {
		...
		if(period!=EnumPeriod.NEVER){
			...
			case DAY:
				if(Calendar.DAY_OF_MONTH!=date.getDay()){
					defaut(true);
					break;
				}
				if(date.getDay()>=days-7&&date.getDay()<=days){
					defaut(true);
					break;
				}
		...
		sql_ip=sql_ip.trim();
		if(sql_ip.length()>0&&sql_ip!="blank"&&sql_ip!=null&&!sql_ip.equalsIgnoreCase("blank")&&!sql_ip.equalsIgnoreCase("null")){
			SQLManager sql = new SQLManager(this,sql_ip,sql_user,sql_pass,sql_db);
			if(block){
				new org.justyce.sql.logger.BlockLogger(sql);
			}
			if(chat){
				new org.justyce.sql.logger.ChatLogger(sql);
			}
			if(command){
				new org.justyce.sql.logger.CommandLogger(sql);
			}
		}
	...
	public void defaut(){
		if(chat){
			chatL =new ChatLogger(this,append,lines);
		}
		if(command){
			commandL = new CommandLogger(this,append,lines);
		}
		if(player){
			playerL = new PlayerLogger(this,append,lines);
		}
		if(vehicle){
			vehicleL = new VehicleLogger(this,append,lines);
		}
		if(craft){
			craftL = new CraftLogger(this,append,lines);
		}
		if(enchantment){
			enchantmentL = new EnchantmentLogger(this,append,lines);
		}
		if(block){
			blockL=new BlockLogger(this,append,lines);
		}
		if(weather){
			weatherL = new WeatherLogger(this,append,lines);
		}
		if(world){
			worldL = new WorldLogger(this,append,lines);
		}
		if(entity){
			entityL = new EntityLogger(this,append,lines);
		}
		if(plugins){
			pluginL = new PluginLogger(this,append,lines);
		}
		if(inventory){
			inventoryL = new InventoryLogger(this,append,lines);
		}
	}
	public void defaut(boolean yes){
		if(yes){
	...
	public void loadConfig(){
		if(!config.exists()){
			new File("./Log").mkdir();
			try {
				log.info(pref+toSuccesString(config.createNewFile())+" "+t.translate("config.created"));
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public void saveConfiguration(boolean first) {
		try {
	...
	private String toSuccesString(boolean c) {
		if(c){
			return "Succesfully";
		}
		else{
				...
				while ((inputLine = in.readLine()) != null) {
					if(inputLine.contains("Latest build :")){
						String last =inputLine.substring(inputLine.indexOf("Lastest build :")+27,
								inputLine.indexOf("1")+8).trim();
						char C =last.charAt(last.length()-1);
						if(C!='0'&&C!='1'&&C!='2'&&C!='3'&&C!='4'&&C!='5'&&C!='6'&&C!='7'&&C!='8'&&C!='9'){
							last=last.substring(0, last.length()-1);
						}
						log.info("[UltraLogger] "+t.translate("update")+" "+last);
					}
				}
				in.close();
			}
